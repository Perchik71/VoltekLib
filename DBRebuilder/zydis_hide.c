...
int zydis_decoder_init()
{
	if (ZYAN_FAILED(ZydisDecoderInit(&_zydis_decoder, ZYDIS_MACHINE_MODE_LONG_64, ZYDIS_ADDRESS_WIDTH_64)))
		return -1;

	return 0;
}

void* zydis_create_instruction()
{
	return malloc(sizeof(ZydisDecodedInstruction));
}

int zydis_release_instruction(void* instruction)
{
	if (instruction)
	{
		free(instruction);
...
int zydis_init_instruction(void* instruction, void* raw)
{
	if (ZYAN_SUCCESS(ZydisDecoderDecodeBuffer(&_zydis_decoder, raw, ZYDIS_MAX_INSTRUCTION_LENGTH, (ZydisDecodedInstruction*)instruction)))
		return 0;

	return -1;
}

int zydis_instruction_length(void* instruction)
{
	return ((ZydisDecodedInstruction*)instruction)->length;
}
